...
class RotaryEmbedding(nn.Module):
    def __init__(self, dim, max_seq_len=1024):
        ...
        # 不参与参数更新（非可训练参数） 的张量（Tensor）注册到模型中：sin和cos是不可被训练的
...
class Attention(nn.Module):
    ...
    def forward(self, hidden_states, use_kv_cache=False):
        ...
        if self.flash_attn:
            ...
            scores = torch.matmul(q, k.transpose(2, 3)) / math.sqrt(self.head_dim) # 计算注意力得分
...
class LLM(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # nn.Embedding相当于权重矩阵，和nn.Liearn区别是它不需要矩阵乘法，直接按idx取出向量作为编码即可
    ...
    def forward(self, input_ids, labels, use_kv_cache=False):
        ...
        # 统一精度为bf16
        hidden_states = hidden_states.to(self.output.weight.dtype)
    ...
    def generate(self, 
        ...
        while input_ids.shape[1] < max_new_tokens - 1:
            ...
            # 如果温度为0，直接取最大概率的输出，否则除以温度后缩小数值差异，再softmax按概率抽取
            if temperature == 0.0:
                _, idx_next = torch.topk(logits, k=1, dim=-1)
            else:
                logits = logits / temperature
                # 如果top_k不为空，取出来最大的k个，将剩余的全部赋值为-inf，后续softmax就可以按概率抽取
